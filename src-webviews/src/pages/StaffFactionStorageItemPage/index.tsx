...
import StaffFactionStorageItem from '../../types/StaffFactionStorageItem';
import ItemTemplate from '../../types/ItemTemplate';

const StaffFactionStorageItemPage = () => {
  const [loading, setLoading] = useState(true);
  const [originalItems, setOriginalItems] = useState<StaffFactionStorageItem[]>([]);
  const [items, setItems] = useState<StaffFactionStorageItem[]>([]);
  const [search, setSearch] = useState('');
  const [modal, setModal] = useState(false);
  const [record, setRecord] = useState<StaffFactionStorageItem>();
  const [templates, setTemplates] = useState<ItemTemplate[]>([]);

  useEffect(() => {
    configureEvent(Constants.STAFF_FACTION_STORAGE_ITEM_PAGE_SHOW, (itemsJson: string) => {
      setOriginalItems(JSON.parse(itemsJson));
      handleModalCancel();
      setLoading(false);
    });

    configureEvent(Constants.STAFF_FACTION_STORAGE_ITEM_PAGE_LOAD_TEMPLATES, (templatesJson: string) => {
    ...
    emitEvent(Constants.STAFF_FACTION_STORAGE_ITEM_PAGE_SHOW);
  }, []);
  ...
  const edit = (record: StaffFactionStorageItem) => {
    setModal(true);
    setRecord(record);
  }
  ...
  const remove = (id: string) => {
    setLoading(true);
    emitEvent(Constants.STAFF_FACTION_STORAGE_ITEM_PAGE_REMOVE, id);
  }

  const handleCancel = () => {
    emitEvent(Constants.STAFF_FACTION_STORAGE_ITEM_PAGE_CLOSE);
  };
  ...
  const handleModalOk = () => {
    setLoading(true);
    emitEvent(Constants.STAFF_FACTION_STORAGE_ITEM_PAGE_SAVE, record.id, record.itemTemplateId, record.quantity, record.price);
  };
  ...
  const columns: ColumnsType<StaffFactionStorageItem> = [
    {
      title: t('name'),
      dataIndex: 'name',
      key: 'name',
    ...
    },
    {
      ...
      key: 'options',
      align: 'center',
      render: (id: string, record: StaffFactionStorageItem) => <Flex justify='space-evenly'>
        ...
        <Row gutter={16}>
          <Col span={24}>
            <Form.Item label={t('price')}>
              <InputNumber value={record.price} onChange={(value) => setRecord({ ...record, price: value })} style={{ width: '100%' }} />
            </Form.Item>
          </Col>
        </Row>
      </Form>
...
export default StaffFactionStorageItemPage;