...
import './base/vehicle';
import { callRemoteEvent, getAddress, getRightCoordsZ } from './base/cursor';
import { activateChat, chatNotifiy } from './base/chat';
import { enterVehicleAsDriver, enterVehicleAsPassenger } from './base/enterVehicles';
import { updateCellphone } from './base/cellphone';
import { Constants } from './base/constants';
import Fingerpointing from './base/fingerpointing';
import { configureEvent, hasPageOpened, setPages, webView, toggleView } from './base/webView';
import { changeFiringMode } from './base/firingMode';
import { detectObject } from './base/staffDoor';
import { toggleMarkers } from './base/marker';
import { handleCharacterEdit } from './base/charCreator';
import { handleTattoosEdit } from './base/tattoos';
import { handleClothesEdit } from './base/clothes';
import { requestAnimDict, requestAnimSet } from './base/animation';

const bombasCombustivel = [
  mp.game.joaat('prop_gas_pump_1a'), mp.game.joaat('prop_gas_pump_1b'), mp.game.joaat('prop_gas_pump_1c'), mp.game.joaat('prop_gas_pump_1d'),
...
mp.keys.bind(Constants.O_KEY, true, () => {
  if (checkKey(Constants.O_KEY))
    return;
...
mp.keys.bind(Constants.F7_KEY, true, () => {
  if (checkKey(Constants.F7_KEY))
    return;
...
mp.keys.bind(Constants.F8_KEY, true, () => {
  if (checkKey(Constants.F8_KEY))
    return;

  const time = mp.game.time.getLocalTime();
  const screenName = `${time.year}-${time.month}-${time.day}-${time.hour}-${time.minute}-${time.second}.png`;
...
const checkKey = (key: number) => {
  return (!player.hasVariable(Constants.PLAYER_META_DATA_NAMETAG) && key !== Constants.F2_KEY)
    ...
    || (player.getVariable('Frozen') && key !== Constants.DELETE_KEY && key !== Constants.F2_KEY && key !== Constants.O_KEY && key !== Constants.F7_KEY && key !== Constants.F8_KEY);
};
...
mp.events.add('RemoveWeapon', (weaponHash: number) => {
  ...
  mp.game.weapon.setPedAmmo(player.handle, weaponHash, 0, false);
});
...
mp.events.add('playerReady', () => {
  ...
  mp.game.hud.setScriptVariableColour(16, 151, 230, 255);
  ...
  mp.discord.update(Constants.SERVER_NAME, Constants.DISCORD_INVITE);
  disableHud();
...
mp.events.add('AddWeaponComponent', (weapon: number, component: number) => {
  mp.game.weapon.giveComponentToPed(player.handle, weapon, component);
});

mp.events.add('RemoveWeaponComponent', (weapon: number, component: number) => {
  mp.game.weapon.removeComponentFromPed(player.handle, weapon, component);
});

mp.events.add('SetDrugEffect', (shakeGameplayCamName: string, shakeGameplayCamIntensity: number, timecycModifier: string, animpostFXName: string) => {
